...
class BallAndSocket(Joint):
    def __init__(self, engine=None, global_position=Vector3(0, 0, 0), global_rotation=RotationMatrix(0, 0, 0), parent=None,
        ...
        if angular_velocity is None:
            angular_velocity = [np.pi / 2, np.pi / 2, np.pi / 2]
        if axis is None:
            axis = [Vector3.right(), Vector3.up(), Vector3.forward()]

        if len(limits) == 1:
            limits = [limits[0], limits[0], limits[0]]
        if len(current_angle) == 1:
            current_angle = [current_angle[0], current_angle[0], current_angle[0]]
        if len(angular_velocity) == 1:
            angular_velocity = [angular_velocity[0], angular_velocity[0], angular_velocity[0]]
        if len(limits) == 1:
            limits = [limits[0], limits[0], limits[0]]

        self.axis = axis
        self.current_angle = [Angle(current_angle[0]), Angle(current_angle[1]), Angle(current_angle[2])]
        self.target_angle = [self.current_angle[0], self.current_angle[1], self.current_angle[2]]
        self.ccw = [True, True, True]
        self.angular_velocity = angular_velocity

        self.limits = limits # angle

        self.delta_angle = [0, 0, 0]
    ...
    def manual_move(self):
        ...
        d_theta[2] = self.angular_velocity[2] * self.engine.delta_time
        ...
        if keys[pygame.K_m]:
            self.ccw[2] = True
            self.set_target_angle(2, self.target_angle[2] + d_theta[2])
        if keys[pygame.K_PERIOD]:
            self.ccw[2] = False
            self.set_target_angle(2, self.target_angle[2] - d_theta[2])
        if keys[pygame.K_COMMA]:
            self.set_target_angle(2, self.current_angle[2])